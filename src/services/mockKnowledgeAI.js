...
(function () {
  ...
  const MOCK_BOOK_NOTE = {
    ...
    summary: 'Resumo gerado a partir da imagem e de resultados simulados de pesquisa. A análise identifica um livro sobre narrativa visual, composição fotográfica e construção de histórias por meio de imagens.',
    keyPoints: [
      ...
      children: [
        {
          ...
          details: {
            title: 'Matemática',
            category: 'Matérias',
            summary: 'Coleção simulada com anotações de matemática voltadas para estudo universitário. O foco está em reconhecer padrões, organizar fórmulas úteis e transformar exercícios em passos objetivos.',
            keyPoints: [
        ...
        },
        {
          ...
          details: {
            title: 'História',
            category: 'Matérias',
            summary: 'Resumo simulado criado para organizar períodos históricos em causas, consequências e personagens centrais. Ideal para transformar imagens de livros ou slides em revisão estruturada.',
            keyPoints: [
      ...
      children: [
        {
          ...
          details: {
            title: 'Python',
            category: 'Programação',
            summary: 'Conteúdo simulado sobre Python para estudos práticos. Reúne conceitos de variáveis, listas, funções e leitura de arquivos com exemplos próximos de exercícios acadêmicos.',
            keyPoints: [
        ...
        },
        {
          ...
          details: {
            title: 'JavaScript',
            category: 'Programação',
            summary: 'Anotação simulada com foco em JavaScript para páginas interativas. O material cobre manipulação de DOM, eventos, armazenamento local e organização de funções.',
            keyPoints: [
          ...
          children: [
            {
              ...
              details: {
                title: 'HTML',
                category: 'Frontend',
                summary: 'Detalhe simulado sobre HTML semântico. Mostra como organizar cabeçalho, navegação, conteúdo principal e seções para tornar páginas mais claras para usuários e tecnologias assistivas.',
                keyPoints: [
            ...
            },
            {
              ...
              details: {
                title: 'CSS',
                category: 'Frontend',
                summary: 'Detalhe simulado sobre CSS aplicado a interfaces responsivas. O conteúdo organiza conceitos de Flexbox, variáveis CSS, estados de botão e adaptação para telas menores.',
                keyPoints: [
      ...
      children: [
        {
          ...
          details: {
            title: 'The Photography Storytelling Workshop',
            category: 'Livros',
            summary: 'Análise simulada de um livro focado em transformar fotografias isoladas em histórias visuais coerentes. O conteúdo sugere atenção a sequência, intenção narrativa, luz e seleção final das imagens.',
            keyPoints: [
  ...
  async function analyzeImageMock(image) {
    await delayMock(850);
  ...
  async function extractTextMock(image) {
    await delayMock(950);
  ...
  async function searchWebMock(query) {
  ...
  async function generateNoteMock({ image, extractedText, searchResults }) {
    ...
    return {
      ...
      summary: MOCK_BOOK_NOTE.summary,
      keyPoints: MOCK_BOOK_NOTE.keyPoints,
      sources: searchResults.length ? searchResults.map(result => ({
        title: result.title,
        url: result.url,
  ...
  function findOrCreateCategoryMock(note, existingCategories = []) {
  ...
  function getInitialHistoryTreeMock() {
    return cloneMock(MOCK_HISTORY_TREE);
  }

  function getHistoryTreeMock() {
    ...
    savedNotes.forEach(note => {
      ...
      const topic = tree.find(item => normalizeCategoryMock(item.title) === normalizeCategoryMock(category))
        || tree.find(item => item.id === 'mock-topic-livros');

      const alreadyListed = topic && topic.children.some(child =>
        child.id === note.id || normalizeCategoryMock(child.title) === normalizeCategoryMock(note.title)
      );
